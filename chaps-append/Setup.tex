% --- Setup Images appendix ---
\FloatBarrier
\chapter{Greenhouse Setup Images}\label{chap:pics}

\begin{figure}[H]  % was [p]
	\centering
	% First row
	\begin{subfigure}[b]{0.28\linewidth}
		\includegraphics[width=\linewidth]{figs/mppt.jpeg}
		\caption{MPPT Controller}
		\label{fig:mppt}
	\end{subfigure}
	\begin{subfigure}[b]{0.28\linewidth}
		\includegraphics[width=\linewidth]{figs/room2.jpeg}
		\caption{Control Room}
		\label{fig:Control Room}
	\end{subfigure}
	\begin{subfigure}[b]{0.28\linewidth}
		\includegraphics[width=\linewidth]{figs/soilsensor.jpeg}
		\caption{Soil Sensor}
		\label{fig:soil_sensor}
	\end{subfigure}
	
	% Second row
	\begin{subfigure}[b]{0.28\linewidth}
		\includegraphics[width=\linewidth]{figs/solar.jpeg}
		\caption{PV Panels}
		\label{fig:pv_panels}
	\end{subfigure}
	\begin{subfigure}[b]{0.28\linewidth}
		\includegraphics[width=\linewidth]{figs/Side.jpeg}
		\caption{Side with Active Vent}
		\label{fig:side_profile}
	\end{subfigure}
	\begin{subfigure}[b]{0.28\linewidth}
		\includegraphics[width=\linewidth]{figs/back.jpeg}
		\caption{Setup Overview}
		\label{fig:setup}
	\end{subfigure}
	
	% Third row
	\begin{subfigure}[b]{0.34\linewidth}
		\includegraphics[width=\linewidth]{figs/mist2.jpeg}
		\caption{Active Misting System}
		\label{fig:mist}
	\end{subfigure}
	\begin{subfigure}[b]{0.25\linewidth}
		\includegraphics[width=\linewidth]{figs/LED.png}
		\caption{LED Lighting}
		\label{fig:led}
	\end{subfigure}
	\begin{subfigure}[b]{0.25\linewidth}
		\includegraphics[width=\linewidth]{figs/LCD2.jpeg}
		\caption{Local Monitoring}
		\label{fig:Local Monitoring}
	\end{subfigure}
	
	\caption{Overview of greenhouse hardware components and setup.}
	\label{fig:hardware_9}
\end{figure}

