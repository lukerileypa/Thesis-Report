...
\begin{figure}[htbp]
	\centering
	\fbox{\includegraphics[width=0.75\textwidth]{figs/soft2.png}}
...
\begin{figure}[H]
	\centering
	\fbox{\includegraphics[width=0.6\textwidth]{figs/soft3.png}}
	\caption{Airflow control}
	\label{fig:airflow_control}
\end{figure}


Airflow control is implemented according to the flowchart shown above. While temperature regulation takes priority over airflow operations, the airflow subsystem functions independently when cooling is not required. In such cases, the dedicated control room fan is activated whenever the ambient temperature inside the greenhouse exceeds 10 °C, ensuring adequate ventilation and preventing electrical components from overheating. Additionaly when temperature control is inactive, the external HAF/intake fan operates intermittently to maintain healthy air circulation and fulfill horizontal-airflow (HAF) requirements within the greenhouse.

\section{Irrigation Control System}
 
\begin{figure}[htbp]
	\centering
	\fbox{\includegraphics[width=0.8\textwidth]{figs/soft4.png}}
...
\begin{figure}[htbp]
	\centering
	\fbox{\includegraphics[width=0.6\textwidth]{figs/soft5.png}}
