...
\begin{equation}
	...
	\;\approx\; \frac{\text{Illuminance (lux)}}{54}
\end{equation}

where 1~\si{\micro\mole\per\metre\squared\per\second} corresponds to roughly 54~lux. 
This relationship allows lux sensor readings to be interpreted in terms of usable light for 
photosynthesis.  \citep{ahn2017}

Monitoring and balancing light intensity through artificial lighting systems and proper greenhouse design is therefore essential to maintain optimal photosynthetic activity without causing plant stress.
...
To support optimal development, research indicates that the ideal relative humidity (RH) range for most greenhouse crops lies between 65\% and 85\%. Maintaining this balance helps prevent both under- and over-transpiration, ensuring steady nutrient uptake and healthy physiological function \cite{pepper_guide}.
...
\ce{CO2} enters plants through stomata via diffusion, and its uptake is influenced by external \ce{CO2} concentration as well as factors such as light, temperature, humidity, and water availability. While plants generally grow well at ambient \ce{CO2} levels (about 340~ppm), concentrations can drop to as low as 200~ppm in sealed greenhouses during the day due to plant uptake. This drop can significantly reduce photosynthesis, sometimes with an effect comparable to the enhancement achieved by raising \ce{CO2} levels to 1,300~ppm.

As seen in the graph below, at low \ce{CO2} concentrations (below approximately 300~ppm), the rate of photosynthesis increases sharply as \ce{CO2} concentration rises. However, beyond approximately 400--500~ppm, further increases in \ce{CO2} concentration have little to no additional effect on the photosynthesis rate, as the curve begins to plateau. Therefore, monitoring and maintaining ambient \ce{CO2} concentrations through simple ventilation is usually sufficient to support healthy plant growth, without the need for costly \ce{CO2} enrichment.

\begin{figure}[h]
    \centering
    \includegraphics[width=0.8\textwidth]{figs/Co2.jpg}
...
This metric provides valuable information for irrigation management, ensuring that plants receive adequate moisture for optimal growth without the risk of overwatering or water stress.


\section{Structural Design Standards}

\subsection{Achitecture of a greenhouse} 
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.8\textwidth]{figs/types.png}
...
The inclination angle significantly affects solar heat gain by altering the amount of radiation absorbed by the greenhouse surface. As shown in the figure below, solar energy gaining rates decrease with steeper angles, particularly for the N--S orientation. While both orientations perform similarly at \SI{17}{\degree}, the E--W orientation consistently captures more energy at higher inclinations. This highlights the importance of optimizing both orientation and tilt to balance heat gain and internal climate control.

\begin{figure}[h]
    \centering
    \includegraphics[width=0.6\textwidth]{figs/inclination.png}
...
Greenhouses are typically classified by structural type and the materials used for framing and covering. The primary structural types include wooden-framed, pipe-framed, and truss-framed greenhouses, each suited to different span widths and construction needs.

\textbf{Wooden-Framed Structures:} Ideal for spans under 6 meters, these greenhouses use timber, often pine or other locally sourced lumber, for columns and side posts. They are cost-effective and reasonably durable for small-scale applications and can also utilize bamboo as an eco-friendly alternative.

\textbf{Pipe-Framed Structures:} Used for medium spans up to approximately 12 meters, these structures incorporate metal pipes to form side posts, columns, cross ties, and purlins. Trusses are generally not included in this design.

\textbf{Truss-Framed Structures:} Suitable for larger spans of 15 meters or more, truss-framed greenhouses offer high structural strength and are commonly used for glasshouse construction. Trusses consist of welded elements like rafters, chords, and struts, made from flat steel, tubular steel, or angle iron. These frames are well-suited for pre-fabrication and large-scale installations.

\textbf{Framing Materials:} Common framing materials include wood, steel, and aluminium. While wood is inexpensive and accessible, aluminium is preferred for its strength, low maintenance, and resistance to corrosion, making it the most durable and cost-effective option for long-term greenhouse use \citep{dalai2020}.

\subsection{Covering materials}
...
The right water system is key to the success of any greenhouse project. As greenhouse plants have a limited soil buffer, precision irrigation systems are vital to ensure high-quality crops and yields \citep{vegtech_netafim}.
...
\textbf{Overhead Sprinkler Systems:} Overhead sprinklers distribute water across the greenhouse from above, providing broad coverage suitable for larger areas. While effective for uniform watering, they may increase humidity levels and wet plant foliage, potentially elevating the risk of disease. Proper management is essential to mitigate these concerns.

\textbf{Misting Systems:} Misting systems emit a fine mist of water into the air, increasing humidity and gently hydrating plants. They are particularly beneficial for delicate seedlings, tropical plants, or crops that thrive in high-humidity environments. Misting helps prevent over-saturation and promotes better air circulation.

\textbf{Ebb and Flow (Flood and Drain) Systems:} Also known as flood and drain systems, ebb and flow setups periodically flood the growing area with nutrient-rich water and then allow it to drain away. This cyclical process ensures that plant roots receive adequate moisture and nutrients while preventing waterlogging. Ebb and flow systems are commonly used in hydroponic greenhouse operations \citep{floraflex_irrigation}.

Irrigation water in these systems is most effectively stored in closed storage tanks. This provides a means to better control water quality as it protects water from outside contaminants, in addition to minimizing loss of water due to evaporation.

\subsection{Heating and Cooling Systems}

\section*{Heating and Cooling Systems in Greenhouses}

Efficient heating and cooling systems are essential for maintaining optimal conditions within greenhouses. These systems can be broadly categorized into \textbf{passive} and \textbf{active} methods, each with distinct mechanisms and applications \citep{vegtech_netafim}.

\textbf{Passive systems} utilize natural processes and strategic design to regulate the greenhouse environment without relying on external energy sources, while \textbf{active systems} involve mechanical or electrical components that require external energy inputs.

\subsection*{Passive Heating}
...
To compensate for temperature drops, especially during winter nights, various heating systems are utilized. Options include unit heaters, central heating systems, radiant heat, and solar heating systems. The choice of system depends on factors such as greenhouse size, crop requirements, and energy efficiency considerations.
...
In situations where passive cooling is insufficient, active methods like evaporative cooling are implemented. This technique involves using fans and wet pads to cool the air through water evaporation, effectively reducing internal temperatures even during hot periods \citep{verma_cooling}.

Integrating both passive and active systems allows for a comprehensive approach to climate control in greenhouses, ensuring optimal growing conditions across varying external temperatures and weather conditions.

\subsection{Humidity Control}

A cost-effective way to achieve humidity control in a controlled environment involves a misting system that sprays mist over the entire crop, thereby increasing humidity~\citep{controlling_humidity}.

It is essential to recognize that relative humidity is strongly influenced by temperature: warm air can hold more moisture than cooler air. As temperatures drop, the relative humidity increases—even if the actual amount of water vapor in the air remains constant.
...
\subsection*{Airflow in Greenhouse Environments}
...
\subsection*{Airflow Components in Greenhouses}
...
One can make use of ThingSpeak with both Raspberry Pi and Arduino to wirelessly upload and visualize sensor data. ThingSpeak is an IoT analytics platform that allows real-time data collection, storage, and visualization from connected devices. Once connected to Wi-Fi, the Arduino or Raspberry Pi can send HTTP requests to ThingSpeak's API to upload sensor readings. By integrating ThingSpeak with these controllers, one can effectively monitor and analyze sensor data remotely, making it a valuable tool for greenhouse automation and other IoT applications \citep{thingspeak}.

\textbf{Actuators and Switches:}

In greenhouse automation, relays are essential components that allow low-voltage control signals to switch high-voltage appliances such as irrigation pumps, lights, and ventilation systems on and off. This ensures efficient and safe operation of the system. These relays can be activated using the low voltage signals sent by the controller \citep{automated_greenhouse_iot_b}.

\section{Energy Efficiency \& Sustainability}

Solar power is useful in greenhouses because it provides a renewable and sustainable energy source for powering equipment, leading to reduced operating costs and environmental impact, while also enabling year-round crop production. By harnessing solar energy, solar-powered greenhouses create sustainable growing conditions for plants, regardless of external climate variations. With a system that runs on the solar power, especially when paired with a battery, it is not necessary to rely on an energy provider or worry about blackouts.
...
\begin{enumerate}
    ...
    \item \textbf{Inverter:} The inverter transforms the DC electricity generated by the solar panels into alternating current (AC) electricity, which is the standard form used to power electrical devices and systems within most greenhouses.
    
    \item \textbf{Charge Controller with Maximum Power Point Tracking (MPPT):} A charge controller manages the flow of electricity from the solar panels to the batteries, preventing overcharging and prolonging battery life. MPPT or Maximum Power Point Tracking is an algorithm included in charge controllers used for extracting maximum available power from a PV module under varying conditions \citep{Eltawil2013}.
    
    \item \textbf{Batteries:} Batteries store the electricity produced by the solar panels, providing energy during periods without sunlight or during power outages, ensuring continuous operation of greenhouse systems.
    
    \item \textbf{Racking and Mounting Systems:} Proper racking and mounting systems are crucial for securely installing solar panels on the greenhouse structure or the ground. These systems ensure that panels are positioned at an optimal angle and orientation to maximize solar exposure. Standard rails and brackets for panels of various sizes exist on the market \citep{axe_struct_solar_mounting}.
\end{enumerate}
